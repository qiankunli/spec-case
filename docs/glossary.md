...
| **unit-id** | 把 spec/一组 case 绑到一个代码符号的稳定标识，格式 `<relpath>::<symbol>`。`specgen` 与 `ccr` 共用的 join key。详见 `specs/unit-id`。 |
| **binding** | case/spec 上的代码绑定字段（`unit-id` + spec 文本）。本仓相对通用 case 模型独有的扩展。 |
...
| **spec.json** | `specgen` 的生成态产物，按 unit-id 索引 `{spec, cases[]}`。`ccr` 的 `SpecBuilder` 入口。 |
