...
- `languages/go.md` · `languages/python.md` — 各语言 spec/case/link 表达（Go `+spec`/`+case`/`+link` marker、Python `@spec`/`@case`/`@link` decorator）+ 示例。
...
- **spec / case / link 都挂 symbol**：一个符号 0..1 个 spec、0..N 个 case、0..N 个 link（link = 作者策展的 "see also"，md 路径或另一 unit-id，见 `docs/concepts.md#link`）。
