...
**边界**：spec-case 定义**资产的形状、表达与绑定契约**（concept / schema / marker grammar / unit-id），并提供**参考实现**——Python 的 marker 装饰器（项目 import）+ `specgen` 抽取器（`python/`，把 marker 编译成 `spec.json`）。**不实现** harness 引擎（黑盒跑 case 那套）。被测仓直接用本仓的 specgen，无需自己实现。
...
- `python/` — ★ **Python 参考实现**：`spec_case`（marker 装饰器，项目 `from spec_case import spec, case, link, rule`）+ `spec_case.specgen`（`ast` 静态抽取 → `spec.json`，`python -m spec_case.specgen <dir>`）。Go 的 specgen 待补。

## 关键约定

- **两个 key 不混**：`case_id`（`^[a-z][a-z0-9_]*$`，CaseSet 内对齐一条 case）；`unit-id`（`<relpath>::<symbol>`，把 spec/一组 case 绑到一个代码符号）。
- **spec / case / link / rule 都挂 symbol**（ccr 评审一个改动函数收的**四类上下文**）：一个符号 0..1 spec、0..N case、0..N link、0..N rule。link = 作者策展的 "see also"（md 路径或另一 unit-id）；rule = 函数级审查准则（rule.json 的共置细化）。见 `docs/concepts.md`。
